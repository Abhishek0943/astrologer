                    ...
                    <button className='otp_btn' style={{ background: "var(--yellow)", color: "var(--dark)", fontWeight: "600", cursor: "pointer" }} onClick={() => {
                        ...
                        if (name.length < 1) {
                            ...
                            dispatch(SinginUser({ name, c: email, password, dob, zodiac, bt, bp, })).then((e) => {
                                e.payload?.success ? dispatch(PopupState({ status: "Success", message: `Verification link Send on ${email} Successfully` })) : dispatch(PopupState({ status: "Error", message: e.payload.message }))
                            })
                        }
                    }}>Submit</button>
                </div>
